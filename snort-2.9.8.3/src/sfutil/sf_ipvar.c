...
SFIP_RET sfvar_add_node(sfip_var_t *var, sfip_node_t *node, int negated)
{
    ...
    if((node->flags & SFIP_ANY) ||
       (sfip_compare(&node->ip->addr, &(*head)->ip->addr) == SFIP_LESSER))
    {
        node->next = *head;
        *head = node;
        return SFIP_SUCCESS;
    ...
    {
        if(sfip_compare(&node->ip->addr, &p->next->ip->addr) == SFIP_LESSER)
        {
            swp = p->next;
            p->next = node;
            node->next = swp;
...
static SFIP_RET sfvar_list_compare(sfip_node_t *list1, sfip_node_t *list2)
{
    ...
    for (tmp = list1; tmp != NULL; tmp = tmp->next)
    {
        ...
        {
            if ((sfip_compare(&tmp->ip->addr, &tmp2->ip->addr) == SFIP_EQUAL) && !usage[i])
            {
                match = 1;
                usage[i] = 1;
                break;
